...
# Helper function for TensorFlow compatibility
def tf_placeholder(*args, **kwargs):
    """Helper function to use the right placeholder based on TensorFlow version"""
    if hasattr(tf, 'compat'):
        return tf.compat.v1.placeholder(*args, **kwargs)
    else:
        return tf.placeholder(*args, **kwargs)

# Helper function for trainable_variables compatibility
def tf_trainable_variables(*args, **kwargs):
    """Helper function for TensorFlow trainable_variables compatibility"""
    if hasattr(tf, 'compat'):
        return tf.compat.v1.trainable_variables(*args, **kwargs)
    else:
        return tf.trainable_variables(*args, **kwargs)

# Helper function for gradients compatibility
def tf_gradients(*args, **kwargs):
    """Helper function for TensorFlow gradients compatibility"""
    if hasattr(tf, 'compat'):
        return tf.compat.v1.gradients(*args, **kwargs)
    else:
        return tf.gradients(*args, **kwargs)

# Helper function for div compatibility
def tf_div(*args, **kwargs):
    """Helper function for TensorFlow div compatibility"""
    return tf.divide(*args, **kwargs)
...
class Trainer(object):
    ...
    def __init__(self, params, best_metric):
        ...
        self.best_threshold = 0.5  # Initialize with default threshold

        # optimize
        self.learning_rate_judge_init = params['learning_rate_judge']
        # Use compatibility layer for TensorFlow 2.x
        if hasattr(tf, 'compat'):
            self.learning_rate_judge_ph = tf.compat.v1.placeholder(tf.float32, shape=(), name="learning_rate_judge") #TODO: Dele the placeholder?
        else:
        ...
        # Use compatibility layer for optimizers
        if hasattr(tf, 'compat'):
            self.optimizer_judge = tf.compat.v1.train.AdamOptimizer(self.learning_rate_judge_ph)
            self.optimizer_agents = tf.compat.v1.train.AdamOptimizer(self.learning_rate_agents)
        else:
    ...
    def calc_reinforce_loss(self):
        ...
        final_reward_1 = tf_div(final_reward_1 - reward_mean_1, reward_std_1)
        final_reward_2 = tf_div(final_reward_2 - reward_mean_2, reward_std_2)
    ...
    def initialize(self, restore=None, sess=None):
        ...
        self.query_subject = tf_placeholder(tf.int32, [None], name="query_subject")
        ...
        self.random_flag = tf_placeholder(tf.bool, [])
        self.range_arr = tf_placeholder(tf.int32, shape=[None, ])
        ...
        self.cum_discounted_reward_1 = tf_placeholder(tf.float32, [None, self.number_steps // 2],
                                                      name="cumulative_discounted_reward")
        self.cum_discounted_reward_2 = tf_placeholder(tf.float32, [None, self.number_steps // 2],
        ...
        for t in range(self.number_steps):
            which_agent = tf_placeholder(tf.float32, shape=(), name="which_agent_{}".format(t))
            next_possible_relations = tf_placeholder(tf.int32, [None, self.max_num_actions],
                                                     name="next_relations_{}".format(t))
            next_possible_entities = tf_placeholder(tf.int32, [None, self.max_num_actions],
                                                    name="next_entities_{}".format(t))

            input_label_relation = tf_placeholder(tf.int32, [None], name="input_label_relation_{}".format(t))
            start_entities = tf_placeholder(tf.int32, [None, ])
        ...
        self.t = tf_placeholder(tf.int32, shape=[], name="time_step")
        self.input_argument = tf_placeholder(tf.float32, name="partial_argument")
        self.input_hidden_argu_rep= tf_placeholder(tf.float32, shape=[None, self.hidden_size], name="rep_argument")
        self.next_relations = tf_placeholder(tf.int32, shape=[None, self.max_num_actions])
        self.next_entities = tf_placeholder(tf.int32, shape=[None, self.max_num_actions])
        self.which_agent = tf_placeholder(tf.float32, name="which_agent")
        self.current_entities = tf_placeholder(tf.int32, shape=[None, ])
        self.prev_relation = tf_placeholder(tf.int32, [None, ], name="previous_relation")

        self.prev_state_agent_1 = tf_placeholder(tf.float32, self.agent.get_mem_shape(), name="memory_of_agent_1")
        formated_state_agent_1 = self.agent.format_state(self.prev_state_agent_1)

        self.prev_state_agent_2 = tf_placeholder(tf.float32, self.agent.get_mem_shape(), name="memory_of_agent_2")
        formated_state_agent_2 = self.agent.format_state(self.prev_state_agent_2)

        self.output_ = tf_placeholder(shape=[None, self.judge.hidden_size], dtype=tf.float32)
        self.test_loss, \
    ...
    def bp_judge(self, loss):
        ...
        tvars = tf_trainable_variables("judge")
        grads = tf_gradients(loss, tvars)
    ...
    def bp(self, loss_1, loss_2):
        ...
        tvars_1 = tf_trainable_variables("MLP_for_policy_1")
        tvars_2 = tf_trainable_variables("MLP_for_policy_2")


        grads_1 = tf_gradients(loss_1, tvars_1)
        grads_2 = tf_gradients(loss_2, tvars_2)
