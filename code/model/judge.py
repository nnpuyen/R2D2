...
# Compatibility helper for placeholder
def tf_placeholder(*args, **kwargs):
    """Helper function for TensorFlow placeholder compatibility"""
    if hasattr(tf, 'compat'):
        return tf.compat.v1.placeholder(*args, **kwargs)
    else:
        return tf.placeholder(*args, **kwargs)

# Compatibility helper for get_variable
def tf_get_variable(*args, **kwargs):
    """Helper function for TensorFlow get_variable compatibility"""
    if hasattr(tf, 'compat'):
...
class Judge():
    ...
    def define_embeddings(self):
        ...
        with tf_variable_scope("judge"):
            self.relation_embedding_placeholder = tf_placeholder(tf.float32,
    ...
    def extend_argument(self, argument, t, action_idx, next_relations, next_entities, range_arr):
        ...
        with tf_variable_scope("judge"):
            with tf_variable_scope("extend_argument"):
    ...
    def classify_argument(self, argument):
        ...
        with tf_variable_scope("judge"):
            with tf_variable_scope("mlp", reuse=get_auto_reuse()):
                hidden = argument
                for i in range(self.hidden_layers - 1):
                    hidden = tf_layers_dense(hidden,self.hidden_size, activation=tf.nn.relu, name="layer_{}".format(i))
                hidden = tf_layers_dense(hidden, self.hidden_size, name='layer_{}'.format(self.hidden_layers - 1))
    ...
    def get_logits_argument(self,argument):
        ...
        with tf_variable_scope("judge"):
            with tf_variable_scope("classifier", reuse=get_auto_reuse()):
                logits = tf_layers_dense(argument, self.hidden_size, name='classifier_judge_0', activation=tf.nn.relu)
                logits = tf_layers_dense(logits,1, name='classifier_judge_1')
        return logits
