...
# Compatibility helper for AUTO_REUSE
def get_auto_reuse():
    """Helper function for TensorFlow AUTO_REUSE compatibility"""
    if hasattr(tf, 'compat'):
        return tf.compat.v1.AUTO_REUSE
    else:
        return tf.AUTO_REUSE

# Compatibility helper for tf.to_int32
def tf_to_int32(x):
...
class Agent(object):
    ...
    def define_embeddings(self):
        ...
        # Use compatibility layer for TensorFlow 2.x
        if hasattr(tf, 'compat'):
            with tf.compat.v1.variable_scope("MLP_for_policy_1"):

                self.relation_embedding_placeholder_agent_1 = tf.compat.v1.placeholder(tf.float32,
                ...
                self.relation_lookup_table_agent_1 = tf.compat.v1.get_variable("relation_lookup_table_agent_1",
                                                             shape=[self.action_vocab_size, self.embedding_size],
                                                             dtype=tf.float32,
                                                             initializer=xavier_init_1,
                ...
                self.entity_embedding_placeholder_agent_1 = tf.compat.v1.placeholder(tf.float32,
                                                                       [self.entity_vocab_size, self.embedding_size])
                self.entity_lookup_table_agent_1 = tf.compat.v1.get_variable("entity_lookup_table_agent_1",
            ...
            with tf_variable_scope("MLP_for_policy_1"):
        ...
        # Use compatibility layer for TensorFlow 2.x
        if hasattr(tf, 'compat'):
            with tf.compat.v1.variable_scope("MLP_for_policy_2"):

                self.relation_embedding_placeholder_agent_2 = tf.compat.v1.placeholder(tf.float32,
                                                                             [self.action_vocab_size, self.embedding_size])
                ...
                self.relation_lookup_table_agent_2 = tf.compat.v1.get_variable("relation_lookup_table_agent_2",
                                                             shape=[self.action_vocab_size, self.embedding_size],
                                                             dtype=tf.float32,
                                                             initializer=xavier_init_2,
                                                             trainable=self.train_relations)
                ...
                self.entity_embedding_placeholder_agent_2 = tf.compat.v1.placeholder(tf.float32,
                                                                       [self.entity_vocab_size, self.embedding_size])
                self.entity_lookup_table_agent_2 = tf.compat.v1.get_variable("entity_lookup_table_agent_2",
                                                               shape=[self.entity_vocab_size, self.embedding_size],
                                                               dtype=tf.float32,
                                                               initializer=self.entity_initializer,
                                                               trainable=self.train_entities)
            ...
            with tf_variable_scope("MLP_for_policy_2"):
    ...
    def policy(self, input_action, which_agent):
        ...
        def policy_1():
            with tf_variable_scope("MLP_for_policy_1", reuse=get_auto_reuse()):
                return self.policy_agent_1(input_action, self.state_agent_1)

        def policy_2():
            with tf_variable_scope("MLP_for_policy_2", reuse=get_auto_reuse()):
    ...
    def action_encoder_agent(self, next_relations, current_entities, which_agent):
        ...
        with tf_variable_scope("lookup_table_edge_encoder"):
    ...
    def step(self, next_relations, next_entities, prev_state_agent_1,
        ...
        # 4 sample action
        action = tf_to_int32(tf_multinomial(logits=scores, num_samples=1))
        action = tf.cond(random_flag, lambda: tf_to_int32(tf_multinomial(logits=uni_scores, num_samples=1)), lambda: action)
